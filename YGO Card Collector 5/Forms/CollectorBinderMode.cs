...
{
    public partial class CollectorBinderMode : Form
    {
        #region Constructors
        public CollectorBinderMode(FormLauncher mainMenuForm)
        {
        ...
        private const int CARDS_PER_PAGE = 9;
        ...
        private void LoadPage()
        {
            ...
            void UpdatePageBanners()
            {
                ...
                if (_MasterCardViewMode)
                {
                    GroupCardView.Text = string.Format("PAGE: {0}/{1} | {2} | Cards: {3}", _CurrentCardPage, lastPage, Database.CardGroupToString(_CurrentCardGroup), _CurrentMasterCardList.Count);
                }
                else
                {
        ...
        protected override bool ProcessCmdKey(ref Message msg, Keys keyData)
        {
            if (keyData == Keys.Up)
            {
                if (_KeyInputEnable)
                {
                    ...
                    if (_CurrentCardImageIndexSelected < 3)
                    {
                    ...
                    else
                    {
                        ...
                        if (_CardPanelList[_CurrentCardImageIndexSelected - 3].Visible)
                        {
                            //change the selection 
                            _CurrentCardImageIndexSelected -= 3;
                            InitializeSelectedCard(_CurrentCardImageIndexSelected);
                        }
                    }
            ...
            else if (keyData == Keys.Right)
            {
                ...
                {
                    _KeyInputEnable = false;
                    if (_CurrentCardImageIndexSelected == 8)
                    {
